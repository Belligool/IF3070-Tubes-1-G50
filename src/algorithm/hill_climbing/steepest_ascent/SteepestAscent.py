...
class SteepestAscent:
    def __init__(self, initial_state, kapasitas, verbose=True):
        """
        Algoritma Steepest Ascent Hill Climbing untuk optimisasi penempatan barang dalam kontainer.
        - initial_state: state awal berisi list kontainer
        - kapasitas: kapasitas tiap kontainer
        - kalau True, cetak log ke terminal
        """
    ...
    def search_best_neighbor(self):
        """Cari neighbor terbaik dari current_state."""
        best_neighbor = None
        ...
        for neighbor in neighbors:
            ...
            if value < best_value:  # karena kita MINIMIZE objective
                best_value = value
    ...
    def run(self):
        """Jalankan algoritma Steepest Ascent sampai tidak ada perbaikan lagi."""
        total_iteration = 0
        ...
        while True:
            ...
            if best_neighbor is not None and best_value < self.current_value:
                ...
                if self.verbose:
                    print(f"Iterasi {total_iteration} → Objective: {self.current_value}")
            else:
                break # ini kalau local maxima tercapai
        ...
        # Menampilkan grafik perkembangan nilai objective
        self.show_plot(
    ...
    def show_plot(objective_values, title="Perkembangan Nilai Objective Function"):
        """Menampilkan grafik perkembangan nilai objective tiap iterasi."""
        if len(objective_values) < 2:
            print("Tidak cukup data untuk menampilkan grafik (iterasi terlalu sedikit).")
            return

        plt.figure(figsize=(10, 6))
        plt.plot(objective_values, label="Objective Value", color="blue", linewidth=2, marker='o')
        plt.title(title)
